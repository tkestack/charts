...
$ helm install my-release --namespace cert-manager --version v1.5.5 jetstack/cert-manager
```
...
```console
$ kubectl delete -f https://github.com/jetstack/cert-manager/releases/download/v1.5.5/cert-manager.crds.yaml
```
...
| `webhook.image.tag` | Webhook image tag | `v1.5.5` |
...
| `cainjector.image.tag` | cainjector image tag | `v1.5.5` |
...
| `startupapicheck.image.tag` | startupapicheck image tag | `v1.5.5` |
