...
本章节详细说明 `values.yaml` 中各配置项的用途、是否必填，以及相关云产品的购买链接。

---

## 配置目录

- [全局配置 (global)](#全局配置-global-1)
  - [基础配置](#基础配置-1)
  - [模型服务配置](#模型服务配置-modelservices-1)
  - [RSA 密钥配置](#rsa-密钥配置-1)
- [基础组件配置 (components)](#基础组件配置-components-1)
  - [数据库配置](#数据库配置-1)
  - [Redis 配置](#redis-配置-1)
  - [Elasticsearch 配置](#elasticsearch-配置-1)
  - [对象存储配置](#对象存储配置-1)
  - [Kafka 消息队列配置](#kafka-消息队列配置-1)
  - [向量数据库配置](#向量数据库配置-1)
- [可观测性配置 (observability)](#可观测性配置-observability-1)
  - [CLS 日志收集配置](#cls-日志收集配置-1)
  - [Prometheus 监控配置](#prometheus-监控配置-1)
  - [APM 链路追踪配置](#apm-链路追踪配置-1)
- [内容安全配置 (contentSecurity)](#内容安全配置-contentsecurity-1)
...
| 配置项 | 必填 | 说明 | 示例值 |
|--------|------|------|--------|
| `clb` | ✅ 必填 | 负载均衡 CLB 域名 | `lb-xxx.clb.bj-tencentclb.com` |
| `clbId` | ✅ 必填 | CLB 实例 ID | `lb-h7m35y2x` |
| `scheme` | ✅ 必填 | 协议类型 | `http` 或 `https` |
...
### 模型服务配置 (modelServices) {#模型服务配置-modelservices-1}
...
|--------|------|------|--------|
| `components.db.vendor` | ✅ 必填 | 云厂商 | `tencent` |
| `components.db.providerType` | ✅ 必填 | 产品类型 | `tdsql` / `cynosdb` / `aurora` / `rds` / `mysql` |
| `components.db.host` | ✅ 必填 | 数据库主机地址 | `192.168.8.81` |
| `components.db.port` | ✅ 必填 | 数据库端口 | `3306` |
| `components.db.user` | ✅ 必填 | 数据库用户名 | `adp-test` |
| `components.db.password` | ✅ 必填 | 数据库密码 | `your-password` |
...
**购买链接：** https://cloud.tencent.com/product/cos
